...
var _deepDiff = function _deepDiff(obj1, obj2, path) {
    obj1 = obj1 || {};
    obj2 = obj2 || {};

    return reduce_1(obj1, function (result, value, key) {
        var p = path ? path + '.' + key : key;
        if (isObject_1(value)) {
            var d = deepDiff(value, obj2[key], p);
            return d.length ? result.concat(d) : result;
        }
        return isEqual_1(value, obj2[key]) ? result : result.concat(p);
    }, []);
};
...
var isVoid = function isVoid(value) {
  var isEmptyObject = _isObject(value) && _isEmptyObject(value);
  var isUndefined = typeof value == 'undefined';
  var isEmptyArray = typeof value == 'array' && value.length == 0;
 ...
 *
 */
var immutableUpdatePayloadTransform = function immutableUpdatePayloadTransform(payload) {
  var _payload$objectAutoVi = payload.objectAutoVivificationCommand,
      objectAutoVivificationCommand = _payload$objectAutoVi === undefined ? '$auto' : _payload$objectAutoVi,
      _payload$arrayAutoViv = payload.arrayAutoVivificationCommand,
      arrayAutoVivificationCommand = _payload$arrayAutoViv === undefined ? '$autoArray' : _payload$arrayAutoViv,
      _payload$action = payload.action,
      action = _payload$action === undefined ? '$set' : _payload$action,
      updateValue = payload.updateValue,
      keyString = payload.keyString,
...
exports.deepDiff = deepDiff$1;
exports.immutableUpdatePayloadTransform = immutableUpdatePayloadTransform;
